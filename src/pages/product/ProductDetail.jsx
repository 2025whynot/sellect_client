...
export default function ProductDetail() {
  ...
  const addToCart = async () => {
    try {
      await put(`${VITE_API_BASE_URL}/api/v1/cart`, {product_id: productId});
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col justify-between">
              <div className="relative">
                <h1 className="text-xl font-extrabold text-gray-900 mb-2">{product.name
                    || "상품명"}</h1>

                <div className="text-sm text-gray-600 mb-4 space-y-1">
                  <p>
                    <span
                        className="font-medium text-gray-800">카테고리:</span> {categoryPath}
                  </p>
                  <p>
                  ...
                  <div className="flex items-center gap-2 relative">
                    ...
                    {showCartMessage && (
                        <div className="absolute bottom-full mb-2 right-0 w-64">
                          <div
                              className="bg-indigo-600 text-white text-sm font-medium px-4 py-2 rounded-lg shadow-lg animate-fade-in-up">
                            <div
                          ...
                          <div
                              className="w-3 h-3 bg-indigo-600 transform rotate-45 absolute right-4 -bottom-1 shadow-lg"></div>
                        </div>
                    )}
                  </div>
                </div>
                <p className="text-gray-700 mb-8">{product.description
                    || "상품 설명이 없습니다."}</p>
              </div>
            </div>
          </div>
          <div className="mt-12">
            <h2 className="text-2xl font-extrabold text-gray-900 mb-4">상품 상세
              정보</h2>
            <div className={`relative space-y-8 ${isExpanded ? ""
